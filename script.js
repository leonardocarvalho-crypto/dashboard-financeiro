...
// Garante o carregamento da biblioteca do Excel (SheetJS) inserindo a tag direto no HTML via script
(function carregarBibliotecaExcel() {
  if (!window.XLSX) {
    ...
    scriptXLSX.onload = () =>
      console.log("Biblioteca XLSX carregada com sucesso via CDN.");
...
function lerPlanilhaExcel(evento) {
  // 🔥 Correção do Erro: Se a biblioteca ainda não carregou no milissegundo do clique, avisa e para
  if (!window.XLSX) {
    alert(
      "A biblioteca de leitura do Excel ainda está carregando. Por favor, aguarde 3 segundos e tente novamente!"
    );
    evento.target.value = "";
    return;
  }
